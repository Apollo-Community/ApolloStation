...
/obj/machinery/power/am_control_unit/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/wrench))
		if(!anchored)
			...
			user << "<span class='alert'>Once bolted and linked to a shielding unit it the [src.name] is unable to be moved!</span>"
		return

	if(istype(W, /obj/item/weapon/am_containment))
		if(fueljar)
			user << "<span class='alert'>There is already a [fueljar] inside!</span>"
			return
		fueljar = W
		W.loc = src
		if(user.client)
