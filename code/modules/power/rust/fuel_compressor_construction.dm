...
/obj/item/rust_fuel_compressor_frame/try_build(turf/on_wall)
	...
	if (!istype(loc, /turf/simulated/floor))
		usr << "<span class='alert'>Compressor cannot be placed on this spot.</span>"
		return
	if (A.requires_power == 0 || A.name == "Space")
		usr << "<span class='alert'>Compressor cannot be placed in this area.</span>"
		return
	...
	if (istype(W, /obj/item/weapon/crowbar))
		if(opened)
			...
			if(compressed_matter > 0)
				user << "<span class='alert'>You cannot open the cover while there is compressed matter inside.</span>"
			else
				opened = 1
		...
		if(do_after(user, 20) && C.amount >= 10)
			C.use(10)
			user.visible_message(\
				"<span class='alert'>[user.name] has added cables to the compressor frame!</span>",\
		...
		if(do_after(user, 50))
			...
			user.visible_message(\
				"<span class='alert'>[user.name] cut the cabling inside the compressor.</span>",\
