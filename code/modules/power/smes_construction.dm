...
/obj/machinery/power/smes/buildable/proc/total_system_failure(var/intensity = 0, var/mob/user as mob)
	...
	message_admins("SMES FAILURE: <b>[src.x]X [src.y]Y [src.z]Z</b> User: [usr.ckey], Intensity: [intensity]/100 - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[src.x];Y=[src.y];Z=[src.z]'>JMP</a>", "LOG:")
	...
	switch (intensity)
		...
		if (61 to INFINITY)
			...
			if (prob(50))
				...
				message_admins("SMES explosion imminent.", "LOG:")
	...
	if (..())
		...
		if(istype(W, /obj/item/weapon/crowbar))
			...
			if (do_after(usr, 100 * cur_coils)) // More coils = takes longer to disassemble. It's complex so largest one with 5 coils will take 50s
				...
				usr << "<span class='alert'> You have disassembled the SMES cell!</span>"
		...
		// Superconducting Magnetic Coil - Upgrade the SMES
			...
			if (cur_coils < max_coils)
				...
				usr << "<span class='alert'> You can't insert more coils to this SMES unit!</span>"
