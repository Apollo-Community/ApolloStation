...
/*
*	Regulates the fusion engine and this components. <rjtwins>
*/
/datum/fusion_controller
	...
	var/chain_broken = 0				//Was the ring chain forcifly broken ?
	var/reactor_chained = 0
...
//Set the chain_broken var to 1
/datum/fusion_controller/proc/chain_broken()
	chain_broken = 1

//Shuts down reactor by controlled gas venting
/datum/fusion_controller/proc/emergencyVent()
	if(gas_contents.temperature >= 90000)
		gas_contents.temperature = 89000	//If we dont lower the temp it will spark and explode.
	leakPlasma()
...
/datum/fusion_controller/proc/checkComponents()
	...
	if(!all_anchored || chain_broken)
		if(!isnull(gas_contents) && gas_contents.total_moles >= 10 && gas_contents.temperature > 90000)
	...
	if(emmag_nr >= fusion_components.len -1)
		for(var/obj/machinery/power/fusion/comp in fusion_components)
			comp.locked = 0
...
/datum/fusion_controller/proc/generatePlasma()
	...
	for(var/obj/machinery/power/fusion/ring/r in fusion_components)
		if(r.outputting)
			p = new(get_step(r, r.dir))
			p.dir = r.dir
			plasma += p
...
//Gets all cores effecting plasma and adds their heat to the general heat.
/datum/fusion_controller/proc/coreHeat()
	var/list/cores = list()
	. = 0
	for(var/obj/machinery/power/fusion/plasma/p in plasma)
		for(var/obj/machinery/power/fusion/core/c in orange(2,p))
			if(get_dist(c,p) != 1)
				continue
			if(cores.Find(c) != 0)
				continue
...
//The new les spageti way
//Self building reactor alternative to hard coded location finder.
/datum/fusion_controller/proc/findComponents()
	world << "calling findComponents"
	.=0	//Any return will return 0
	var/list/temp_list = list()
	//Locate a ring to start from. Gotta be able to designate this somehow
	var/obj/machinery/power/fusion/ring/start = locate(/obj/machinery/power/fusion/ring)
	if(isnull(start) || !istype(start, /obj/machinery/power/fusion/ring))
		return
	var/obj/machinery/power/fusion/prev = start
	prev.start_point = 1
	//Find all connected pieces
	do
		world << prev
		prev = prev.get_next(prev)
		if(isnull(prev))
			return
		temp_list.Add(prev)
		world << "[prev == start ? "start found":""]"
	while(!prev.start_point)

	fusion_components = temp_list
	for(var/obj/machinery/power/fusion/comp in fusion_components)
		comp.in_chain = 1
	world << "findComponents list print len = [fusion_components.len]"
	for(var/obj/machinery/power/fusion/comp in fusion_components)
		world << comp
	reactor_chained = 1
	return 1
