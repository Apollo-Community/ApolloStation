...
/datum/preferences/proc/savePreferences()
	if( !client )
		log_debug( "Failed to save client prefs because theres no client!" )
		return 0

	if( IsGuestKey( client.ckey ))
		log_debug( "Failed to save [client] client prefs because the client is a guest!" )
		return 0

	establish_db_connection()
	if( !dbcon.IsConnected() )
		log_debug( "Failed to save [client] client prefs because the database isnt connected!" )
		return 0
	...
	if(sql_id)
		...
		if(!isnum(sql_id))
			log_debug( "Failed to save [client] client prefs because the index isnt a number!" )
			return 0
