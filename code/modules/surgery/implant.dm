...
/datum/surgery_step/cavity/make_space
	...
	fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		var/datum/organ/external/chest/affected = target.get_organ(target_zone)
		user.visible_message("<span class='alert'> [user]'s hand slips, scraping tissue inside [target]'s [affected.display_name] with \the [tool]!</span>", \
		"<span class='alert'> Your hand slips, scraping tissue inside [target]'s [affected.display_name] with \the [tool]!</span>")
...
/datum/surgery_step/cavity/place_item
	...
	end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		...
		if (tool.w_class > get_max_wclass(affected)/2 && prob(50))
			user << "<span class='alert'> You tear some blood vessels trying to fit such a big object in this cavity.</span>"
	...
	fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		...
		if (affected.implants.len)
			...
			if (prob(fail_prob))
				...
				user.visible_message("<span class='alert'> Something beeps inside [target]'s [affected.display_name]!</span>")
