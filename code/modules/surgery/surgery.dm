...
proc/do_surgery(mob/living/carbon/M, mob/living/user, obj/item/tool)
	...
	if(zone in M.op_stage.in_progress) //Can't operate on someone repeatedly.
		user << "<span class='alert'>You can't operate on this area while surgery is already in progress.</span>"
		return 1
		...
		if(S.tool_quality(tool))
			...
			if(step_is_valid && S.is_valid_target(M))
				...
				if(prob(S.tool_quality(tool)) &&  do_mob(user, M, rand(S.min_duration, S.max_duration)))
					...
					user << "<span class='alert'>You must remain close to your patient to conduct surgery.</span>"
				M.op_stage.in_progress -= zone 									// Clear the in-progress flag.
				return	1	  												//don't want to do weapony things after surgery

	if (user.a_intent == I_HELP)
		user << "<span class='alert'>You can't see any useful way to use [tool] on [M].</span>"
		return 1
	return 0
