...
/obj/item/clothing/suit/space/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/stack/sheet/mineral/plastic) || istype(W,/obj/item/stack/sheet/metal))
		...
		if(istype(src.loc,/mob/living))
			user << "<span class='alert'> How do you intend to patch a hardsuit while someone is wearing it?</span>"
			return
		...
		if(!WT.remove_fuel(5))
			user << "<span class='alert'> You need more welding fuel to repair this suit.</span>"
			return
...
/obj/item/clothing/suit/space/examine(mob/user)
	...
	if(can_breach && breaches && breaches.len)
		for(var/datum/breach/B in breaches)
			user << "<span class='alert'> <B>It has \a [B.descriptor].</B></span>"
