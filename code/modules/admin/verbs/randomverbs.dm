...
/client/proc/respawn_character()
	...
	if(record_found)//If they have a record we can determine a few things.
		new_character.character.name = record_found.fields["name"]
		new_character.character.gender = record_found.fields["sex"]
		new_character.character.age = record_found.fields["age"]
		new_character.character.blood_type = record_found.fields["b_type"]
	else
	...
	*/
	...
	switch(new_character.mind.antagonist)
		if("traitor")
		...
		if("Ninja")
			...
			if(ninjastart.len == 0)
				...
				new_character << "<B><span class='alert'>Attempting to place at a carpspawn.</B></span>"
				for(var/obj/effect/landmark/L in landmarks_list)
					if(L.name == "carpspawn")
						ninjastart.Add(L)
				if(ninjastart.len == 0 && latejoin.len > 0)
					new_character << "<B><span class='alert'>Still no spawneable locations could be found. Defaulting to latejoin.</B></span>"
		...
		if("Death Commando")//Leaves them at late-join spawn.
			...
			switch(new_character.mind.assigned_role)
				if("Cyborg")//More rigging to make em' work and check if they're traitor.
					...
					if(new_character.mind.antagonist=="traitor")
...
/proc/get_mob_info(var/mob/M, mob/user as mob)
	...
	//Job + antagonist
	if(M.mind)
		special_role_description = "Role: <b>[M.mind.assigned_role]</b>; Antagonist: <font color='red'><b>[M.mind.antagonist]</b></font>; Has been rev: [(M.mind.has_been_rev)?"Yes":"No"]"
	else
	...
	user << "<b>Info about [M.name]:</b> "
	user << "Mob type = [M.type]; Gender = [gender_description] Damage = [health_description]"
