...
/client/proc/SDQL_query(query_text as message)
	...
	if(!check_rights(R_DEBUG))  //Shouldn't happen... but just to be safe.
		message_admins("<span class='alert'>ERROR: Non-admin [usr.key] attempted to execute a SDQL query!</span>")
	...
	if(query_list.len < 2)
		if(query_list.len > 0)
			usr << "<span class='alert'>SDQL: Too few discrete tokens in query \"[query_text]\". Please check your syntax and try again.</span>"
		return

	if(!(lowertext(query_list[1]) in list("select", "delete", "update")))
		usr << "<span class='alert'>SDQL: Unknown query type: \"[query_list[1]]\" in query \"[query_text]\". Please check your syntax and try again.</span>"
		return
	...
	if(lowertext(query_list[1]) == "update")
		if(i <= query_list.len && lowertext(query_list[i]) == "set")
			for(i++; i <= query_list.len; i++)
				if(i + 2 <= query_list.len && query_list[i + 1] == "=")
					...
					usr << "<span class='alert'>SDQL: Invalid set parameter in query \"[query_text]\". Please check your syntax and try again.</span>"
					return
		...
		if(set_vars.len < 1)
			usr << "<span class='alert'>SDQL: Invalid or missing set in query \"[query_text]\". Please check your syntax and try again.</span>"
			return
	...
	for(var/datum/t in objs)
		...
		for(i = 1, i - 1 < where.len, i++)
			...
			if(!(compare_op in list("==", "=", "<>", "<", ">", "<=", ">=", "!=")))
				usr << "<span class='alert'>SDQL: Unknown comparison operator [compare_op] in where clause following [v] in query \"[query_text]\". Please check your syntax and try again.</span>"
				return
...
/proc/SDQL_evaluate(datum/object, list/equation)
	if(equation.len == 0)
		...
		usr << "<span class='alert'>SDQL: Sorry, equations not yet supported :(</span>"
		return null
...
/proc/SDQL_tokenize(query_text)
	...
	for(var/i = 1, i <= len, i++)
		...
		if(char in whitespace)
			...
			if(word != "")
				usr << "<span class='alert'>SDQL: You have an error in your SDQL syntax, unexpected ' in query: \"<font color=gray>[query_text]</font>\" following \"<font color=gray>[word]</font>\". Please check your syntax, and try again.</span>"
				return null
			...
			if(i > len)
				usr << "<span class='alert'>SDQL: You have an error in your SDQL syntax, unmatched ' in query: \"<font color=gray>[query_text]</font>\". Please check your syntax, and try again.</span>"
				return null
