...
/mob/living/carbon/human/proc/equip_vox_raider()
	...
	equip_to_slot_or_qdel(new /obj/item/clothing/under/vox/vox_robes(src), slot_w_uniform)
	equip_to_slot_or_qdel(new /obj/item/clothing/shoes/magboots/vox(src), slot_shoes) // REPLACE THESE WITH CODED VOX ALTERNATIVES.
	equip_to_slot_or_qdel(new /obj/item/clothing/gloves/yellow/vox(src), slot_gloves) // AS ABOVE.

	switch(vox_tick)
		if(1) // Vox raider!
			equip_to_slot_or_qdel(new /obj/item/clothing/suit/space/vox/carapace(src), slot_wear_suit)
			equip_to_slot_or_qdel(new /obj/item/clothing/head/helmet/space/vox/carapace(src), slot_head)
			equip_to_slot_or_qdel(new /obj/item/weapon/melee/baton/loaded(src), slot_belt)
			equip_to_slot_or_qdel(new /obj/item/clothing/glasses/thermal/monocle(src), slot_glasses) // REPLACE WITH CODED VOX ALTERNATIVE.
			equip_to_slot_or_qdel(new /obj/item/device/chameleon(src), slot_l_store)

			var/obj/item/weapon/gun/launcher/spikethrower/W = new(src)
			equip_to_slot_or_qdel(W, slot_r_hand)
		...
		if(2) // Vox engineer!
			...
			equip_to_slot_or_qdel(new /obj/item/weapon/storage/box/emps(src), slot_r_hand)
	...
	equip_to_slot_or_qdel(new /obj/item/clothing/mask/breath(src), slot_wear_mask)
	equip_to_slot_or_qdel(new /obj/item/weapon/tank/nitrogen(src), slot_back)
	equip_to_slot_or_qdel(new /obj/item/device/flashlight(src), slot_r_store)
