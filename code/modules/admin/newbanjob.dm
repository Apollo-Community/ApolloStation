...
//		AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, "Geneticist")
	...
	if ( Banlistjob.dir.Find("[ckey][computerid][rank]") )
		usr << text("<span class='alert'> Banjob already exists.</span>")
		return 0
	else
...
/datum/admins/proc/unjobbanpanel()
	var/count = 0
	var/dat
	//var/dat = "<HR><B>Unban Player:</B> <span class='notice'>(U</span>) = Unban , (E) = Edit Ban\green (Total<HR><table border=1 rules=all frame=void cellspacing=0 cellpadding=3 >"
	Banlistjob.cd = "/base"
	for (var/A in Banlistjob.dir)
		count++
		Banlistjob.cd = "/base/[A]"
...
/*/datum/admins/proc/permjobban(ckey, computerid, reason, bannedby, temp, minutes, rank)
	if(AddBanjob(ckey, computerid, reason, usr.ckey, 0, 0, job))
		M << "<span class='alert'><BIG><B>You have been banned from [job] by [usr.client.ckey].\nReason: [reason].</B></BIG></span>"
...
/datum/admins/proc/timejobban(ckey, computerid, reason, bannedby, temp, minutes, rank)
	if(AddBanjob(ckey, computerid, reason, usr.ckey, 1, mins, job))
		M << "<span class='alert'><BIG><B>You have been jobbanned from [job] by [usr.client.ckey].\nReason: [reason].</B></BIG></span>"
