...
/datum/artifact_effect/robohurt/DoEffectTouch(var/mob/user)
	if(user)
		if (istype(user, /mob/living/silicon/robot))
			...
			R << "<span class='alert'>Your systems report severe damage has been inflicted!</span>"
	...
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				M << "<span class='alert'>SYSTEM ALERT: Harmful energy field detected!</span>"
...
/datum/artifact_effect/robohurt/DoEffectPulse()
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				M << "<span class='alert'>SYSTEM ALERT: Structural damage inflicted by energy pulse!</span>"
				last_message = world.time
