...
/datum/artifact_effect/hurt/DoEffectTouch(var/mob/toucher)
	if(toucher)
		...
		if(iscarbon(toucher) && prob(weakness * 100))
			...
			C << "<span class='alert'>A painful discharge of energy strikes you!</span>"
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			...
			if(prob(weakness * 100))
				if(prob(10))
					C << "<span class='alert'>You feel a painful force radiating from something nearby.</span>"
...
/datum/artifact_effect/hurt/DoEffectPulse()
	if(holder)
		...
		for (var/mob/living/carbon/C in range(effectrange, T))
			var/weakness = GetAnomalySusceptibility(C)
			if(prob(weakness * 100))
				C << "<span class='alert'>A wave of painful energy strikes you!</span>"
