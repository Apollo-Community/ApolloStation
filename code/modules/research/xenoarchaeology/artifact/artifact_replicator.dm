...
/obj/machinery/replicator/New()
	...
	fail_message = "<span class='notice'> \icon[src] a [pick("loud</span>","soft","sinister","eery","triumphant","depressing","cheerful","angry")] \
...
/obj/machinery/replicator/process()
	if(spawning_types.len && powered())
		...
		if(spawn_progress_time > max_spawn_time)
			...
			src.visible_message("<span class='notice'> \icon[src] [src] [pick("clicks</span>","whizzes","whirrs","whooshes","clanks","clongs","clonks","bangs")].")
		...
		if(index > 0 && index <= construction.len)
			if(stored_materials.len > spawning_types.len)
				if(spawning_types.len)
					src.visible_message("<span class='notice'> \icon[src] a [pick("light</span>","dial","display","meter","pad")] on [src]'s front [pick("blinks","flashes")] [pick("red","yellow","blue","orange","purple","green","white")].")
				else
