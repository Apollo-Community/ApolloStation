	...
	if (suspension_field)
		...
		if(field_type == "carbon")
			for(var/mob/living/carbon/M in T)
				...
				cell.charge -= power_use
				if(prob(5))
					M << "<span class='notice'> [pick("You feel tingly.</span>","You feel like floating.","It is hard to speak.","You can barely move.")]"
...
/obj/machinery/suspension_gen/verb/rotate_ccw()
	...
	if(anchored)
		usr << "<span class='alert'> You cannot rotate [src], it has been firmly fixed to the floor.</span>"
	else
		set_dir(turn(dir, 90))
