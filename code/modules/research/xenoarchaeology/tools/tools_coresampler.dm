...
/obj/item/device/core_sampler/examine(mob/user)
	if(..(user, 2))
		user << "<span class='notice'>Used to extract geological core samples - this one is [sampled_turf ? "full" : "empty</span>"], and has [num_stored_bags] bag[num_stored_bags != 1 ? "s" : ""] remaining."
...
/obj/item/device/core_sampler/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/weapon/evidencebag))
		if(num_stored_bags < 10)
			...
			user << "<span class='alert'>The core sampler can not fit any more bags!</span>"
	else
		return ..()
...
/obj/item/device/core_sampler/proc/sample_item(var/item_to_sample, var/mob/user as mob)
	...
	if(geo_data)
		if(filled_bag)
			user << "<span class='alert'>The core sampler is full!</span>"
		else if(num_stored_bags < 1)
			user << "<span class='alert'>The core sampler is out of sample bags!</span>"
		else
		...
		user << "<span class='alert'>You are unable to take a sample of [item_to_sample].</span>"
...
/obj/item/device/core_sampler/attack_self()
	if(filled_bag)
		...
		usr << "<span class='alert'>The core sampler is empty.</span>"
