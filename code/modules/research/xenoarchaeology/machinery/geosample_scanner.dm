...
/obj/machinery/radiocarbon_spectrometer/process()
	if(scanning)
		if(!scanned_item || scanned_item.loc != src)
			...
			if(prob(5))
				src.visible_message("<span class='notice'> \icon[src] [pick("whirrs</span>","chuffs","clicks")][pick(" excitedly"," energetically"," busily")].", 2)
	else
		//gradually cool down over time
		if(scanner_temperature > 0)
			scanner_temperature = max(scanner_temperature - 5 - 10 * rand(), 0)
		if(prob(0.75))
			src.visible_message("<span class='notice'> \icon[src] [pick("plinks</span>","hisses")][pick(" quietly"," softly"," sadly"," plaintively")].", 2)
