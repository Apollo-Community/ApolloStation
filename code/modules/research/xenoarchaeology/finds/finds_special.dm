...
/obj/item/weapon/vampiric/process()
	...
	if(charges >= 0.1)
		if(prob(5))
			src.visible_message("<span class='alert'> \icon[src] [src]'s eyes glow ruby red for a moment!</span>")
			charges -= 0.1
...
/obj/item/weapon/vampiric/proc/bloodcall(var/mob/living/carbon/human/M)
	...
	if(istype(M))
		...
		M << "<span class='alert'> The skin on your [parse_zone(target)] feels like it's ripping apart, and a stream of blood flies out.</span>"
...
/obj/effect/shadow_wight/Bump(var/atom/obstacle)
	obstacle << "<span class='alert'> You feel a chill run down your spine!</span>"
