...
/mob/living/simple_animal/hostile/retaliate/malf_drone/Life()
	...
	if(prob(1))
		src.visible_message("<span class='alert'> \icon[src] [src] shudders and shakes as some of it's damaged systems come back online.</span>")
	...
	if(prob(disabled ? 0 : 1))
		if(hostile_drone)
			...
			src.visible_message("<span class='alert'> \icon[src] [src] suddenly lights up, and additional targetting vanes slide into place.</span>")
			hostile_drone = 1
	...
	if(exploding && prob(20))
		if(prob(50))
			src.visible_message("<span class='alert'> \icon[src] [src] begins to spark and shake violenty!</span>")
		else
			src.visible_message("<span class='alert'> \icon[src] [src] sparks and shakes like it's about to explode!</span>")
		var/datum/effect/effect/system/spark_spread/s = new /datum/effect/effect/system/spark_spread
		s.set_up(3, 1, src)
		s.start()
...
/mob/living/simple_animal/hostile/retaliate/malf_drone/Destroy()
	...
	if(has_loot)
		...
		//plasteel
		O = new /obj/item/stack/sheet/alloy/plasteel(src.loc)
		step_to(O, get_turf(pick(view(7, src))))
		if(prob(75))
			O = new /obj/item/stack/sheet/alloy/plasteel(src.loc)
			step_to(O, get_turf(pick(view(7, src))))
