...
/obj/vehicle/train/MouseDrop_T(var/atom/movable/C, mob/user as mob)
	...
	if(istype(C,/obj/vehicle/train))
		...
		if(!load(C))
			user << "<span class='alert'>You were unable to load [C] on [src].</span>"
...
/obj/vehicle/train/proc/attach_to(obj/vehicle/train/T, mob/user)
	if (get_dist(src, T) > 1)
		user << "<span class='alert'>[src] is too far away from [T] to hitch them together.</span>"
		return

	if (lead)
		user << "<span class='alert'>[src] is already hitched to something.</span>"
		return

	if (T.tow)
		user << "<span class='alert'>[T] is already towing something.</span>"
		return
	...
	while (next_car)
		if (next_car == src)
			user << "<span class='alert'>That seems very silly.</span>"
			return
...
/obj/vehicle/train/proc/unattach(mob/user)
	if (!lead)
		user << "<span class='alert'>[src] is not hitched to anything.</span>"
		return
