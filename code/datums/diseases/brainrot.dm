...
/datum/disease/brainrot/stage_act() //Removed toxloss because damaging diseases are pretty horrible. Last round it killed the entire station because the cure didn't work -- Urist
	...
	switch(stage)
		if(2)
			...
			if(prob(2))
				affected_mob << "<span class='alert'> Your don't feel like yourself.</span>"
			if(prob(5))
		...
		if(3)
			...
			if(prob(10) && affected_mob.getBrainLoss()<=98)//shouldn't retard you to death now
				...
				affected_mob.updatehealth()
				if(prob(2))
					affected_mob << "<span class='alert'> Your try to remember something important...but can't.</span>"
/*			if(prob(10))
...
/*			if(prob(15))
			...
			if(prob(15) && affected_mob.getBrainLoss()<=98) //shouldn't retard you to death now
				...
				affected_mob.updatehealth()
				if(prob(2))
					affected_mob << "<span class='alert'> Strange buzzing fills your head, removing all thoughts.</span>"
			if(prob(3))
				affected_mob << "<span class='alert'> You lose consciousness...</span>"
