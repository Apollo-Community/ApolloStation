			...
			switch(href_list["monkey"])
				if("healthy")
					if (usr.client.holder.rights & R_ADMIN)
						...
						if (istype(H))
							...
							message_admins("\blue [key_name_admin(usr)] attempting to monkeyize [key_name_admin(current)]", "LOG:")
							src = null
							M = H.monkeyize()
							src = M.mind
				...
				if("infected")
					if (usr.client.holder.rights & R_ADMIN)
						...
						if (istype(H))
							...
							message_admins("\blue [key_name_admin(usr)] attempting to monkeyize and infect [key_name_admin(current)]", "LOG:")
							src = null
							M = H.monkeyize()
							src = M.mind
				...
				if("human")
					...
					if (istype(M))
						...
						message_admins("\blue [key_name_admin(usr)] attempting to humanize [key_name_admin(current)]", "LOG:")
