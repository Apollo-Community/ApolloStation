...
/proc/error(msg)
	...
	STUI.cached_logs[6] += "\[[time_stamp()]]ERROR: [msg]<br>"

#define WARNING(MSG) warning("[MSG] in [__FILE__] at line [__LINE__] src: [src] usr: [usr].")
//print a warning message to world.log
/proc/warning(msg)
	world.log << "## WARNING: [msg][log_end]"
	STUI.cached_logs[6] += "\[[time_stamp()]]WARNING: [msg]<br>"

//print a testing-mode debug message to world.log
/proc/testing(msg)
	world.log << "## TESTING: [msg][log_end]"
	STUI.cached_logs[6] += "\[[time_stamp()]]TEST: [msg]<br>"
...
/proc/log_debug(text)
	...
	STUI.cached_logs[6] += "\[[time_stamp()]]DEBUG: [text]<br>"
...
/proc/log_game(text)
	...
	STUI.cached_logs[2] += "\[[time_stamp()]]GAME: [text]<br>"
...
/proc/log_access(text)
	...
	STUI.cached_logs[6] += "\[[time_stamp()]]ACCESS: [text]<br>"

/proc/log_say(text)
	if (config.log_say)
		diary << "\[[time_stamp()]]SAY: [text][log_end]"
	STUI.cached_logs[5] += "\[[time_stamp()]]SAY: [text]<br>"
...
/proc/log_whisper(text)
	...
	STUI.cached_logs[5] += "\[[time_stamp()]]WHISPER: [text]<br>"

/proc/log_emote(text)
	if (config.log_emote)
		diary << "\[[time_stamp()]]EMOTE: [text][log_end]"
	STUI.cached_logs[5] += "\[[time_stamp()]]<font color='#999999'>EMOTE: [text]</font><br>"

/proc/log_attack(text)
	if (config.log_attack)
		diary << "\[[time_stamp()]]ATTACK: [text][log_end]" //Seperate attack logs? Why?  FOR THE GLORY OF SATAN!
	STUI.cached_logs[1] += "\[[time_stamp()]]ATTACK: [text]<br>"
...
/proc/log_pda(text)
	...
	STUI.cached_logs[5] += "\[[time_stamp()]]<font color='#cd6500'>PDA: [text]</font><br>"

/proc/log_misc(text)
	diary << "\[[time_stamp()]]MISC: [text][log_end]"
	STUI.cached_logs[6] += "\[[time_stamp()]]MISC: [text]<br>"
