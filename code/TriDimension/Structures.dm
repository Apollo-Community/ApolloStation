	...
	attack_hand(var/mob/M)
		...
		if(!target || !istype(target.loc, /turf))
			...
			if(blocked || istype(T, /turf/simulated/wall))
				...
				M.visible_message("<span class='notice'>\The [M] climbs [src.icon_state == "ladderup" ? "up" : "down</span>"] \the [src]!", "You climb [src.icon_state == "ladderup"  ? "up" : "down"] \the [src]!", "You hear some grunting, and clanging of a metal ladder being used.")
				M.Move(target.loc)
	...
	attack_hand(var/mob/M)
		...
		spawn(7)
			...
			if(M.z == z && get_dist(src,M) <= 1)
				...
				M.visible_message("<span class='notice'>\The [M] scurries [target.z == adjacent_to_me["up"] ? "up" : "down</span>"] \the [src]!", "You scramble [target.z == adjacent_to_me["up"] ? "up" : "down"] \the [src]!", "You hear some grunting, and a hatch sealing.")
				M.Move(target.loc)
