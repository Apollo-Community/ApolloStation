...
/datum/job/captain/equip(var/mob/living/carbon/human/H)
	...
	if(H.character.age>49)		U.hastie = new /obj/item/clothing/accessory/medal/gold/captain(U)
...
/datum/job/captain/make_preview_icon( var/backpack , var/job , var/gender )
	var/icon/clothes_s = null

	clothes_s = new /icon('icons/mob/uniform.dmi', "captain_s")
	clothes_s.Blend(new /icon('icons/mob/feet.dmi', "brown"), ICON_UNDERLAY)

	if(prob(1))		clothes_s.Blend(new /icon('icons/mob/head.dmi', "centcomcaptain"), ICON_OVERLAY)
	else			clothes_s.Blend(new /icon('icons/mob/head.dmi', "captain"), ICON_OVERLAY)
