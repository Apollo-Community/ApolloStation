	...
	if (user != O)
		for(var/mob/B in viewers(user, 3))
			if ((B.client && !( B.blinded )))
				B << text("<span class='alert'> [] stuffs [] into []!</span>", user, O, src)
	return
...
//		return
	if (cremating)
		usr << "<span class='alert'> It's locked.</span>"
		return
...
//		return
	...
	if(contents.len <= 0)
		for (var/mob/M in viewers(src))
			M.show_message("<span class='alert'> You hear a hollow crackle.</span>", 1)
			return
		...
		for (var/mob/M in viewers(src))
			M.show_message("<span class='alert'> You hear a roar as the crematorium activates.</span>", 1)
