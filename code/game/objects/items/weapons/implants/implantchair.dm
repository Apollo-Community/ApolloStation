...
/obj/machinery/implantchair
	...
	put_mob(mob/living/carbon/M as mob)
		if(!iscarbon(M))
			usr << "<span class='alert'><B>The [src.name] cannot hold this!</B></span>"
			return
		if(src.occupant)
			usr << "<span class='alert'><B>The [src.name] is already occupied!</B></span>"
			return
		if(M.client)
		...
		for(var/obj/item/weapon/implant/loyalty/imp in implant_list)
			...
			if(istype(imp, /obj/item/weapon/implant/loyalty))
				for (var/mob/O in viewers(M, null))
					O.show_message("<span class='alert'>[M] has been implanted by the [src.name].</span>", 1)
