...
/obj/item/weapon/autopsy_scanner/verb/print_data()
	...
	for(var/mob/O in viewers(usr))
		O.show_message("<span class='alert'> \the [src] rattles and prints out a sheet of paper.</span>", 1)
...
/obj/item/weapon/autopsy_scanner/attack(mob/living/carbon/human/M as mob, mob/living/carbon/user as mob)
	...
	if(target_name != M.name)
		...
		user << "<span class='alert'> A new patient has been registered.. Purging data for previous patient.</span>"
	...
	for(var/mob/O in viewers(M))
		O.show_message("<span class='alert'> [user.name] scans the wounds on [M.name]'s [S.display_name] with \the [src.name]</span>", 1)
