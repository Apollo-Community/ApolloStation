...
/obj/item/weapon/storage/briefcase/attack(mob/living/M as mob, mob/living/user as mob)
	...
	if (M.stat < 2 && M.health < 50 && prob(90))
		...
		if ((istype(H, /mob/living/carbon/human) && istype(H, /obj/item/clothing/head) && H.flags & 8 && prob(80)))
			M << "<span class='alert'>The helmet protects you from being hit hard in the head!</span>"
			return
		var/time = rand(2, 6)
		if (prob(75))
			M.Paralyse(time)
		...
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span class='alert'><B>[] has been knocked unconscious!</B></span>", M), 1, "<span class='alert'>You hear someone fall.</span>", 2)
	else
		M << text("<span class='alert'>[] tried to knock you unconcious!</span>",user)
		M.eye_blurry += 3
