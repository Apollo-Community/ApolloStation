			...
			if ((istype(W, /obj/item/device/multitool)) && (src.open == 1)&& (!src.l_hacking))
				...
				if (do_after(usr, 100))
					if (prob(40))
						src.l_setshort = 1
						src.l_set = 0
						user.show_message(text("<span class='alert'> Internal memory reset.  Please give it a few seconds to reinitialize.</span>"), 1)
						sleep(80)
						src.l_setshort = 0
						src.l_hacking = 0
					else
						user.show_message(text("<span class='alert'> Unable to reset internal memory.</span>"), 1)
						src.l_hacking = 0
...
/obj/item/weapon/storage/secure/briefcase
	...
	attack_hand(mob/user as mob)
		if ((src.loc == user) && (src.locked == 1))
			usr << "<span class='alert'> [src] is locked and cannot be opened!</span>"
			...
			if(ishuman(M))
				...
				// ******* Check
					...
					for(var/mob/O in viewers(H, null))
						O.show_message(text("<span class='alert'> <B>[] has been knocked unconscious!</B></span>", H), 1, "<span class='alert'> You hear someone fall.</span>", 2)
				else
					H << text("<span class='alert'> [] tried to knock you unconcious!</span>",user)
					H.eye_blurry += 3
