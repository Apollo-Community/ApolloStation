...
/obj/item/toy/balloon/throw_impact(atom/hit_atom)
	if(src.reagents.total_volume >= 1)
		src.visible_message("<span class='alert'>The [src] bursts!</span>","You hear a pop and a splash.")
...
/obj/item/toy/gun
	...
	attackby(obj/item/toy/ammo/gun/A as obj, mob/user as mob)
		...
		if (istype(A, /obj/item/toy/ammo/gun))
			if (src.bullets >= 7)
				...
				return 1
			if (A.amount_left <= 0)
				user << "<span class='alert'>There is no more caps!</span>"
				return 1
			if (A.amount_left < (7 - src.bullets))
				src.bullets += A.amount_left
				user << text("<span class='alert'>You reload [] caps\s!</span>", A.amount_left)
				A.amount_left = 0
			else
				user << text("<span class='alert'>You reload [] caps\s!</span>", 7 - src.bullets)
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		...
		for(var/mob/O in viewers(user, null))
			O.show_message(text("<span class='alert'><B>[] fires a cap gun at []!</B></span>", user, target), 1, "<span class='alert'>You hear a gunshot</span>", 2)
	...
	attackby(obj/item/I as obj, mob/user as mob)
		if(istype(I, /obj/item/toy/ammo/crossbow))
			if(bullets <= 4)
				...
				usr << "<span class='alert'>It's already fully loaded.</span>"
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		...
		if (locate (/obj/structure/table, src.loc))
				...
				if (D)
					...
					for(var/mob/living/M in D.loc)
						...
						for(var/mob/O in viewers(world.view, D))
							O.show_message(text("<span class='alert'>[] was hit by the foam dart!</span>", M), 1)
			...
			for(var/mob/O in viewers(world.view, user))
				O.show_message(text("<span class='alert'>[] realized they were out of ammo and starting scrounging for some!</span>", user), 1)
...
// ******* Check
		...
		if (src.bullets > 0 && M.lying)
			...
			for(var/mob/O in viewers(M, null))
				if(O.client)
					O.show_message(text("<span class='alert'><B>[] casually lines up a shot with []'s head and pulls the trigger!</B></span>", user, M), 1, "<span class='alert'>You hear the sound of foam against skull</span>", 2)
					O.show_message(text("<span class='alert'>[] was hit in the head by the foam dart!</span>", M), 1)
			...
			for(var/mob/O in viewers(M, null))
				if (O.client)	O.show_message(text("<span class='alert'><B>[] casually lines up a shot with []'s head, pulls the trigger, then realizes they are out of ammo and drops to the floor in search of some!</B></span>", user, M), 1, "<span class='alert'>You hear someone fall</span>", 2)
			user.Weaken(5)
		return
...
/obj/item/toy/crayon
	...
	suicide_act(mob/user)
		viewers(user) << "<span class='alert'><b>[user] is jamming the [src.name] up \his nose and into \his brain. It looks like \he's trying to commit suicide.</b></span>"
...
/obj/item/toy/snappop
	...
	throw_impact(atom/hit_atom)
		...
		src.visible_message("<span class='alert'>The [src.name] explodes!</span>","<span class='alert'>You hear a snap!</span>")
...
/obj/item/toy/snappop/Crossed(H as mob|obj)
	if((ishuman(H))) //i guess carp and shit shouldn't set them off
		...
		if(M.m_intent == "run")
			M << "<span class='alert'>You step on the snap pop!</span>"
		...
		spawn(0)
			for(var/i=0, i<1, i++)
				...
				for(var/atom/T in get_turf(D))
					...
					if(ismob(T) && T:client)
						T:client << "<span class='alert'>[user] has sprayed you with water!</span>"
				sleep(4)
			qdel(D)
