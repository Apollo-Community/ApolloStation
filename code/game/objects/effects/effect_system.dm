...
/datum/effect/effect/system/lightning_bolt/New()
	..()
/datum/effect/effect/system/lightning_bolt/start(atom/scourse, atom/target)
	var/vector/start = new (scourse.x * world.icon_size, scourse.y * world.icon_size)
	var/vector/dest  = new (target.x * world.icon_size, target.y * world.icon_size)
	var/bolt/b = new(start, dest, 50)
	b.Draw(scourse.z, color = "#ffffff", thickness = 2, split = 1)
			...
			spawn(0)
				...
				if(T != src.oldposition)
					if(istype(T, /turf/space))
						var/obj/effect/effect/ion_trails/I = PoolOrNew( /obj/effect/effect/ion_trails, list( src.oldposition ))
						src.oldposition = T
	...
	if(src.processing)
		...
		spawn(0)
			...
			if(currloc != T)
				...
				if(istype(T, /turf/space))
					var/obj/effect/effect/ion_trails/I = PoolOrNew( /obj/effect/effect/ion_trails, list( src.oldposition ))
					var/obj/effect/effect/ion_trails/II = PoolOrNew( /obj/effect/effect/ion_trails, list( src.oldloc ))
					//src.oldposition = T
...
/datum/effect/effect/system/steam_trail_follow
	...
	start()
		...
		if(src.processing)
			...
			spawn(0)
				if(src.number < 3)
					var/obj/effect/effect/steam/I = PoolOrNew( /obj/effect/effect/steam, list( src.oldposition ))
					src.number++
...
/obj/effect/effect/foam/New(loc, var/ismetal=0)
	...
	spawn(120)
		...
		if(metal)
			var/obj/structure/foamedmetal/M = PoolOrNew( /obj/structure/foamedmetal, list( src.loc ))
			M.metal = metal
			M.updateicon()
		...
		if ((HULK in user.mutations) || (prob(75 - metal*25)))
			...
			for(var/mob/O in oviewers(user))
				if ((O.client && !( O.blinded )))
					O << "<span class='alert'>[user] smashes through the foamed metal.</span>"
	...
	attackby(var/obj/item/I, var/mob/user)
		...
		if (istype(I, /obj/item/weapon/grab))
			...
			for(var/mob/O in viewers(src))
				if (O.client)
					O << "<span class='alert'>[G.assailant] smashes [G.affecting] through the foamed metal wall.</span>"
			qdel(I)
			qdel(src)
			return
	...
	start()
		if (amount <= 2)
			...
			for(var/mob/M in viewers(5, location))
				M << "<span class='alert'>The solution violently explodes.</span>"
			for(var/mob/M in viewers(1, location))
				if (prob (50 * amount))
					M << "<span class='alert'>The explosion knocks you down.</span>"
					M.Weaken(rand(1,5))
			return
		else
			var/devastation = -1
