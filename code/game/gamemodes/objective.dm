...
datum/objective
	...
	proc/find_target_by_role(role, role_type=0)//Option sets either to check assigned role or special role. Default to assigned.
		for(var/datum/mind/possible_target in ticker.minds)
			if((possible_target != owner) && ishuman(possible_target.current) && ((role_type ? possible_target.antagonist : possible_target.assigned_role) == role) )
...
datum/objective/assassinate
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Assassinate [target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist]."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/mutiny/rp
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Assassinate, capture or convert [target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist]."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/anti_revolution/execute
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "[target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist] has extracted confidential information above their clearance. Execute \him[target.current]."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/anti_revolution/brig
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Brig [target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist] for 20 minutes to set an example."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/anti_revolution/demote
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "[target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist] has been classified as harmful to NanoTrasen's goals. Demote \him[target.current] to assistant."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/debrain//I want braaaainssss
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Steal the brain of [target.current.real_name] the [!role_type ? target.assigned_role : target.antagonist]."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/protect//The opposite of killing a dude.
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Protect [target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist]."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/brig
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Have [target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist] brigged for 10 minutes."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/harm
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Make an example of [target.current.real_name], the [!role_type ? target.assigned_role : target.antagonist]. Break one of their bones, detach one of their limbs or disfigure their face. Make sure they're alive when you do it."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/steal
	...
	var/global/possible_items[] = list(
		...
		"an ansible crystal" = /obj/item/weapon/stock_parts/subspace/crystal,
