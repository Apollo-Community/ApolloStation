...
datum/objective/assassinate
	...
	check_completion()
		if(target && target.current)
			if((target.current.stat == DEAD) || issilicon(target.current) || isbrain(target.current) || (target.current.z in overmap.admin_levels) || !target.current.ckey) //Borgs/brains/AIs count as dead for traitor objectives. --NeoFite
				return 1
			return 0
		return 1
