...
/datum/game_mode/proc/equip_traitor(mob/living/carbon/human/traitor_mob, var/safety = 0)
	...
	if(traitor_mob.client.prefs.selected_character.uplinklocation == "Headset")
	...
	else if(traitor_mob.client.prefs.selected_character.uplinklocation == "PDA")
		R = locate(/obj/item/device/pda) in traitor_mob.contents
		if(!R)
			R = locate(/obj/item/device/radio) in traitor_mob.contents
	...
	else if(traitor_mob.client.prefs.selected_character.uplinklocation == "None")
