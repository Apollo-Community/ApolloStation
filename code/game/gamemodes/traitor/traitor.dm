...
/datum/game_mode/proc/forge_traitor_objectives(var/datum/mind/traitor)
	...
	if(istype(traitor.current, /mob/living/silicon))
		var/datum/objective/targeted/assassinate/kill_objective = new
		...
		switch(rand(1,100))
			if(1 to 33)
				traitor.objectives += new /datum/objective/targeted/assassinate( traitor )
			if(34 to 50)
				traitor.objectives += new /datum/objective/targeted/brig( traitor )
			if(51 to 66)
				traitor.objectives += new /datum/objective/targeted/harm( traitor )
			else
				var/steal_objective = pickRandomObjective( /datum/objective/steal )
				traitor.objectives += new steal_objective( traitor )
		switch(rand(1,100))
			if(1 to 100)
