			...
			if(0)
				if(istype(O,/obj/item/weapon/weldingtool))
					...
					if (WT.get_fuel() < 5) // uses up 5 fuel.
						user << "<span class='alert'> You need more fuel to complete this task.</span>"
						return
...
/obj/machinery/nuclearbomb/attack_hand(mob/user as mob)
	if (src.extended)
		...
		if(removal_stage < 5)
			src.anchored = 1
			visible_message("<span class='alert'> With a steely snap, bolts slide out of [src] and anchor it to the flooring!</span>")
		else
			visible_message("<span class='alert'> \The [src] makes a highly unpleasant crunching noise. It looks like the anchoring bolts have been cut.</span>")
		if(!src.lighthack)
			flick("nuclearbombc", src)
...
/obj/machinery/nuclearbomb/verb/make_deployable()
	...
	if (src.deployable)
		usr << "<span class='alert'> You close several panels to make [src] undeployable.</span>"
		src.deployable = 0
	else
		usr << "<span class='alert'> You adjust some panels to make [src] deployable.</span>"
		src.deployable = 1
	return
			...
			if (src.yes_code)
				...
				if (href_list["timer"])
					if (src.timing == -1.0)
						return
					if (src.safety)
						usr << "<span class='alert'> The safety is still on.</span>"
						return
				...
				if (href_list["anchor"])
					...
					if(src.anchored)
						...
						visible_message("<span class='alert'> The anchoring bolts slide back into the depths of [src].</span>")
