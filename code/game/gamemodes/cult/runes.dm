...
/////////////////////////////////////////FIRST RUNE
	proc
		...
		//	var/tempnum = 0
			...
			if(index >= 5)
				user << "<span class='alert'>You feel pain, as rune disappears in reality shift caused by too much wear of space-time fabric</span>"
			...
			if(allrunesloc && index != 0)
				...
				user.visible_message("<span class='alert'>[user] disappears in a flash of red light!</span>", \
				"<span class='alert'>You feel as your body gets dragged through the dimension of Nar-Sie!</span>", \
				"<span class='alert'>You hear a sickening crunch and sloshing of viscera.</span>")
...
//			var/index = 0
		//	var/tempnum = 0
			...
			if(culcount>=3)
				...
				user.visible_message("<span class='alert'>You feel air moving from the rune - like as it was swapped with somewhere else.</span>", \
				"<span class='alert'>You feel air moving from the rune - like as it was swapped with somewhere else.</span>", \
				"<span class='alert'>You smell ozone.</span>")
...
/////////////////////////////////////////SECOND RUNE
		...
		tomesummon()
			...
			usr.visible_message("<span class='alert'>Rune disappears with a flash of red light, and in its place now a book lies.</span>", \
			"<span class='alert'>You are blinded by the flash of red light! After you're able to see again, you see that now instead of the rune there's a book.</span>", \
			"<span class='alert'>You hear a pop and smell ozone.</span>")
			if(istype(src,/obj/effect/rune))
		...
		convert()
			for(var/mob/living/carbon/M in src.loc)
				...
				if (M.species && (M.species.flags & NO_PAIN))
					M.visible_message("<span class='alert'>The markings below [M] glow a bloody red.</span>")
				else
					M.visible_message("<span class='alert'>[M] writhes in pain as the markings below \him glow a bloody red.</span>", \
					"<span class='alert'>AAAAAAHHHH!</span>", \
					"<span class='alert'>You hear an anguished scream.</span>")
				...
				if(R.word1==cultwords["travel"] && R.word2==cultwords["blood"] && R.word3==cultwords["self"])
					for(var/mob/living/carbon/D in R.loc)
						if(D.stat!=2)
							...
							D << "<span class='alert'>You feel weakened.</span>"
							D.take_overall_damage(bdrain, 0)
							drain += bdrain
			if(!drain)
				return fizzle()
			usr.say ("Yu[pick("'","`")]gular faras desdae. Havas mithum javara. Umathar uf'kal thenar!")
			usr.visible_message("<span class='alert'>Blood flows from the rune into [usr]!</span>", \
			"<span class='alert'>The blood starts flowing from the rune and into your frail mortal body. You feel... empowered.</span>", \
			"<span class='alert'>You hear a liquid flowing.</span>")
			var/mob/living/user = usr
			if(user.bhunger)
				user.bhunger = max(user.bhunger-2*drain,0)
			if(drain>=50)
				user.visible_message("<span class='alert'>[user]'s eyes give off eerie red glow!</span>", \
				"<span class='alert'>...but it wasn't nearly enough. You crave, crave for more. The hunger consumes you from within.</span>", \
				"<span class='alert'>You hear a heartbeat.</span>")
				user.bhunger += drain
				src = user
				spawn()
		...
		seer()
			if(usr.loc==src.loc)
				if(usr.seer==1)
					...
					usr << "<span class='alert'>The world beyond fades from your vision.</span>"
					usr.see_invisible = SEE_INVISIBLE_LIVING
					usr.seer = 0
				else if(usr.see_invisible!=SEE_INVISIBLE_LIVING)
					usr << "<span class='alert'>The world beyond flashes your eyes but disappears quickly, as if something is disrupting your vision.</span>"
					usr.see_invisible = SEE_INVISIBLE_OBSERVER
					usr.seer = 0
				else
					usr.say("Rash'tla sektath mal[pick("'","`")]zua. Zasan therium vivira. Itonis al'ra matum!")
					usr << "<span class='alert'>The world beyond opens to your eyes.</span>"
					usr.see_invisible = SEE_INVISIBLE_OBSERVER
					usr.seer = 1
				return
			return fizzle()
		...
		raise()
			...
			if(!corpse_to_raise)
				if(is_sacrifice_target)
					usr << "<span class='alert'>The Geometer of blood wants this mortal for himself.</span>"
				return fizzle()
			...
			if(!body_to_sacrifice)
				if (is_sacrifice_target)
					usr << "<span class='alert'>The Geometer of blood wants that corpse for himself.</span>"
				else
					usr << "<span class='alert'>The sacrifical corpse is not dead. You must free it from this world of illusions before it may be used.</span>"
				return fizzle()
			...
			if(!ghost)
				usr << "<span class='alert'>You require a restless spirit which clings to this world. Beckon their prescence with the sacred chants of Nar-Sie.</span>"
				return fizzle()
			...
			corpse_to_raise.visible_message("<span class='alert'>[corpse_to_raise]'s eyes glow with a faint red as he stands up, slowly starting to breathe again.</span>", \
			"<span class='alert'>Life... I'm alive again...</span>", \
			"<span class='alert'>You hear a faint, slightly familiar whisper.</span>")
			body_to_sacrifice.visible_message("<span class='alert'>[body_to_sacrifice] is torn apart, a black smoke swiftly dissipating from his remains!</span>", \
			"<span class='alert'>You feel as your blood boils, tearing you apart.</span>", \
			"<span class='alert'>You hear a thousand voices, all crying in pain.</span>")
			...
			if(S)
				if(istype(src,/obj/effect/rune))
					...
					for (var/mob/V in viewers(src))
						V.show_message("<span class='alert'>The rune turns into gray dust, veiling the surrounding runes.</span>", 3)
					qdel(src)
				else
					usr.whisper("Kla[pick("'","`")]atu barada nikt'o!")
					usr << "<span class='alert'>Your talisman turns into gray dust, veiling the surrounding runes.</span>"
					for (var/mob/V in orange(1,src))
						if(V!=usr)
							V.show_message("<span class='alert'>Dust emanates from [usr]'s hands for a moment.</span>", 3)
...
/////////////////////////////////////////TENTH RUNE
		...
		ajourney() //some bits copypastaed from admin tools - Urist
			if(usr.loc==src.loc)
				...
				usr.visible_message("<span class='alert'>[usr]'s eyes glow blue as \he freezes in place, absolutely motionless.</span>", \
				"<span class='alert'>The shadow that is your spirit separates itself from your body. You are now in the realm beyond. While this is a great sight, being here strains your mind and body. Hurry...</span>", \
				"<span class='alert'>You hear only complete silence for a moment.</span>")
...
/////////////////////////////////////////ELEVENTH RUNE
		...
		manifest()
			...
			usr.visible_message("<span class='alert'>A shape forms in the center of the rune. A shape of... a man.</span>", \
			"<span class='alert'>A shape forms in the center of the rune. A shape of... a man.</span>", \
			"<span class='alert'>You hear liquid flowing.</span>")
			D.real_name = "Unknown"
			var/chose_name = 0
			...
			D.character.skin_tone = 35
			D.character.eye_color = rgb( 255, 50, 50 )
			D.character.underwear = 0
			...
			while(this_rune && user && user.stat==CONSCIOUS && user.client && user.loc==this_rune.loc)
				...
				sleep(30)
			if(D)
				D.visible_message("<span class='alert'>[D] slowly dissipates into dust and bones.</span>", \
				"<span class='alert'>You feel pain, as bonds formed between your soul and this homunculus break.</span>", \
				"<span class='alert'>You hear faint rustle.</span>")
				D.dust()
			return
		...
		talisman()//only hide, emp, teleport, deafen, blind and tome runes can be imbued atm
			...
			if (!newtalisman)
				if (unsuitable_newtalisman)
					usr << "<span class='alert'>The blank is tainted. It is unsuitable.</span>"
				return fizzle()
			...
			if (imbued_from)
				for (var/mob/V in viewers(src))
					V.show_message("<span class='alert'>The runes turn into dust, which then forms into an arcane image on the paper.</span>", 3)
...
/////////////////////////////////////////THIRTEENTH RUNE
		...
		mend()
			...
			runedec+=10
			user.visible_message("<span class='alert'>[user] keels over dead, his blood glowing blue as it escapes his body and dissipates into thin air.</span>", \
			"<span class='alert'>In the last moment of your humble life, you feel an immense pain as fabric of reality mends... with your blood.</span>", \
			"<span class='alert'>You hear faint rustle.</span>")
			for(,user.stat==2)
				sleep(600)
				if (!user)
...
/////////////////////////////////////////FOURTEETH RUNE
		...
		communicate()
			...
			for(var/datum/mind/H in ticker.mode.cult)
				if (H.current)
					H.current << "<span class='alert'>\b [input]</span>"
			qdel(src)
			return 1
					...
					if(H.mind == ticker.mode:sacrifice_target)
						if(cultsinrange.len >= 3)
							...
							if(isrobot(H))
								...
								H.gib()
							usr << "<span class='alert'>The Geometer of Blood accepts this sacrifice, your objective is now complete.</span>"
						else
							usr << "<span class='alert'>Your target's earthly bonds are too strong. You need more cultists to succeed in this ritual.</span>"
					else
						if(cultsinrange.len >= 3)
							if(H.stat !=2)
								if(prob(80) || worth)
									usr << "<span class='alert'>The Geometer of Blood accepts this [worth ? "exotic " : ""]sacrifice.</span>"
									ticker.mode:grant_runeword(usr)
								else
									usr << "<span class='alert'>The Geometer of blood accepts this sacrifice.</span>"
									usr << "<span class='alert'>However, this soul was not enough to gain His favor.</span>"
								if(isrobot(H))
									H.dust()//To prevent the MMI from remaining
								else
									H.gib()
								...
								if(prob(40) || worth)
									...
									usr << "<span class='alert'>The Geometer of blood accepts this sacrifice.</span>"
									usr << "<span class='alert'>However, a mere dead body is not enough to satisfy Him.</span>"
								if(isrobot(H))
									H.dust()//To prevent the MMI from remaining
								else
									H.gib()
							...
							if(H.stat !=2)
								usr << "<span class='alert'>The victim is still alive, you will need more cultists chanting for the sacrifice to succeed.</span>"
							else
								if(prob(40))
									usr << "<span class='alert'>The Geometer of blood accepts this sacrifice.</span>"
									ticker.mode:grant_runeword(usr)
...
/////////////////////////////////////////SIXTEENTH RUNE
		...
		revealrunes(var/obj/W as obj)
			...
			if(S)
				if(istype(W,/obj/item/weapon/nullrod))
					usr << "<span class='alert'>Arcane markings suddenly glow from underneath a thin layer of dust!</span>"
					return
				if(istype(W,/obj/effect/rune))
					usr.say("Nikt[pick("'","`")]o barada kla'atu!")
					for (var/mob/V in viewers(src))
						V.show_message("<span class='alert'>The rune turns into red dust, reveaing the surrounding runes.</span>", 3)
					qdel(src)
					return
				if(istype(W,/obj/item/weapon/paper/talisman))
					usr.whisper("Nikt[pick("'","`")]o barada kla'atu!")
					usr << "<span class='alert'>Your talisman turns into red dust, revealing the surrounding runes.</span>"
					for (var/mob/V in orange(1,usr.loc))
						if(V!=usr)
							V.show_message("<span class='alert'>Red dust emanates from [usr]'s hands for a moment.</span>", 3)
					return
				return
			if(istype(W,/obj/effect/rune))
				return	fizzle()
...
/////////////////////////////////////////SEVENTEENTH RUNE
		...
		wall()
			...
			if(src.density)
				usr << "<span class='alert'>Your blood flows into the rune, and you feel that the very space over the rune thickens.</span>"
			else
				usr << "<span class='alert'>Your blood flows into the rune, and you feel as the rune releases its grasp on space.</span>"
			return
		...
		freedom()
			...
			if(users.len>=3)
				...
				if(!(cultist.buckled || \
					...
					user << "<span class='alert'>The [cultist] is already free.</span>"
					return
		...
		cultsummon()
			...
			if(users.len>=3)
				...
				if(cultist.buckled || cultist.handcuffed || (!isturf(cultist.loc) && !istype(cultist.loc, /obj/structure/closet)))
					user << "<span class='alert'>You cannot summon \the [cultist], for his shackles of blood are strong.</span>"
					return fizzle()
				cultist.loc = src.loc
				cultist.lying = 1
				...
				user.visible_message("<span class='alert'>Rune disappears with a flash of red light, and in its place now a body lies.</span>", \
				"<span class='alert'>You are blinded by the flash of red light! After you're able to see again, you see that now instead of the rune there's a body.</span>", \
				"<span class='alert'>You hear a pop and smell ozone.</span>")
				qdel(src)
			return fizzle()
			...
			if(istype(src,/obj/effect/rune))
				...
				for(var/mob/living/carbon/C in range(7,src))
					...
					if(N)
						continue
					C.ear_deaf += 50
					C.show_message("<span class='alert'>The world around you suddenly becomes quiet.</span>", 3)
					affected++
					if(prob(1))
						C.sdisabilities |= DEAF
				if(affected)
					usr.say("Sti[pick("'","`")] kaliedir!")
					usr << "<span class='alert'>The world becomes quiet as the deafening rune dissipates into fine dust.</span>"
					qdel(src)
				else
					return fizzle()
			else
				...
				if(affected)
					...
					usr << "<span class='alert'>Your talisman turns into gray dust, deafening everyone around.</span>"
					for (var/mob/V in orange(1,src))
						if(!(iscultist(V)))
							V.show_message("<span class='alert'>Dust flows from [usr]'s hands for a moment, and the world suddenly becomes quiet..</span>", 3)
			return
		...
		blind()
			if(istype(src,/obj/effect/rune))
				...
				for(var/mob/living/carbon/C in viewers(src))
					...
					C.show_message("<span class='alert'>Suddenly you see red flash that blinds you.</span>", 3)
					affected++
				if(affected)
					usr.say("Sti[pick("'","`")] kaliesin!")
					usr << "<span class='alert'>The rune flashes, blinding those who not follow the Nar-Sie, and dissipates into fine dust.</span>"
					qdel(src)
				else
					return fizzle()
			else
				...
				for(var/mob/living/carbon/C in view(2,usr))
					...
					//talismans is weaker.
					affected++
					C.show_message("<span class='alert'>You feel a sharp pain in your eyes, and the world disappears into darkness..</span>", 3)
				if(affected)
					usr.whisper("Sti[pick("'","`")] kaliesin!")
					usr << "<span class='alert'>Your talisman turns into gray dust, blinding those who not follow the Nar-Sie.</span>"
			return
...
//			var/list/cultboil = list(cultists-usr) //and for this words are destroy-see-blood.
			...
			if(culcount>=3)
				for(var/mob/living/carbon/M in viewers(usr))
					...
					M << "<span class='alert'>Your blood boils!</span>"
					if(prob(5))
						spawn(5)
							M.gib()
					...
					if(R.blood_DNA == src.blood_DNA)
						for(var/mob/living/M in orange(2,R))
							...
							if (R.invisibility>M.see_invisible)
								M << "<span class='alert'>Aargh it burns!</span>"
							else
								M << "<span class='alert'>Rune suddenly ignites, burning you!</span>"
				...
				for(var/obj/effect/decal/cleanable/blood/B in world)
					if(B.blood_DNA == src.blood_DNA)
						for(var/mob/living/M in orange(1,B))
							...
							M << "<span class='alert'>Blood suddenly ignites, burning you!</span>"
			...
			if(istype(src,/obj/effect/rune))   ///When invoked as rune, flash and stun everyone around.
				...
				for(var/mob/living/L in viewers(src))
					...
					if(iscarbon(L))
						...
						C.Weaken(1)
						C.Stun(1)
						C.show_message("<span class='alert'>The rune explodes in a bright flash.</span>", 3)

					else if(issilicon(L))
						var/mob/living/silicon/S = L
						S.Weaken(5)
						S.show_message("<span class='alert'>BZZZT... The rune has exploded in a bright flash.</span>", 3)
				qdel(src)
				...
				if(N)
					for(var/mob/O in viewers(T, null))
						O.show_message(text("<span class='alert'><B>[] invokes a talisman at [], but they are unaffected!</B></span>", usr, T), 1)
				else
					for(var/mob/O in viewers(T, null))
						O.show_message(text("<span class='alert'><B>[] invokes a talisman at []</B></span>", usr, T), 1)
...
/////////////////////////////////////////TWENTY-FIFTH RUNE
		...
		armor()
			...
			usr.visible_message("<span class='alert'>The rune disappears with a flash of red light, and a set of armor appears on [usr]...</span>", \
			"<span class='alert'>You are blinded by the flash of red light! After you're able to see again, you see that you are now wearing a set of armor.</span>")
