	...
	proc
		fizzle()
			...
			for (var/mob/V in viewers(src))
				V.show_message("<span class='alert'> The markings pulse with a small burst of light, then fall dark.</span>", 3, "<span class='alert'> You hear a faint fizzle.</span>", 2)
			return
...
//		usr << browse(null, "window=tank")
	...
	attack(mob/living/M as mob, mob/living/user as mob)
		...
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span class='alert'> <B>[] beats [] with the arcane tome!</B></span>", user, M), 1)
		M << "<span class='alert'> You feel searing heat inside!</span>"
	...
	attack_self(mob/living/user as mob)
		...
		if(iscultist(user))
			...
			if (!istype(user.loc,/turf))
				user << "<span class='alert'> You do not have enough space to write a proper rune.</span>"
				return
...
//						call(/obj/item/weapon/book/tome/proc/edit_notes)()
			...
			if(usr)
				...
				if (!chosen_rune)
					return
				if (chosen_rune == "none")
					user << "<span class='alert'> You decide against scribing a rune, perhaps you should take this time to study your notes.</span>"
					return
			...
			for (var/mob/V in viewers(src))
				V.show_message("<span class='alert'> [user] slices open a finger and begins to chant and paint symbols on the floor.</span>", 3, "<span class='alert'> You hear chanting.</span>", 2)
			user << "<span class='alert'> You slice open one of your fingers and begin drawing a rune on the floor whilst chanting the ritual that binds your life essence with the dark arcane energies flowing through the surrounding world.</span>"
			...
			if(do_after(user, 50))
				...
				user << "<span class='alert'> You finish drawing the arcane markings of the Geometer.</span>"
