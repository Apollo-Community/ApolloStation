...
/obj/machinery/apiary/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/queen_bee))
		if(health > 0)
			user << "<span class='alert'> There is already a queen in there.</span>"
		else
			health = 10
			nutrilevel += 10
			user.drop_item()
		...
		if(health > 0)
			user << "<span class='alert'> <b>You begin to dislodge the apiary from the tray, the bees don't like that.</b></span>"
			angry_swarm(user)
		else
		...
		if(do_after(user, 50))
			...
			user << "<span class='alert'> You dislodge the apiary from the tray.</span>"
			qdel(src)
		...
		if(harvestable_honey > 0)
			if(health > 0)
				user << "<span class='alert'> You begin to harvest the honey. The bees don't seem to like it.</span>"
				angry_swarm(user)
			else
