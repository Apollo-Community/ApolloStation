...
/obj/machinery/computer/message_monitor/attack_hand(var/mob/living/user as mob)
	...
	switch(screen)
		...
		if(0)
			...
			if((istype(user, /mob/living/silicon/ai) || istype(user, /mob/living/silicon/robot)) && (user.mind.antagonist && user.mind.original == user))
...
/obj/machinery/computer/message_monitor/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if (href_list["hack"])
			if((istype(usr, /mob/living/silicon/ai) || istype(usr, /mob/living/silicon/robot)) && (usr.mind.antagonist && usr.mind.original == usr))
				src.hacking = 1
				src.screen = 2
