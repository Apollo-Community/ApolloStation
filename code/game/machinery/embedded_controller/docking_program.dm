...
/datum/computer/file/embedded_program/docking/process()
	var/debOOg = 0
	if(id_tag in list("escape_shuttle", "centcom_dock"))
		debOOg = 1
	if(debOOg) testing("[id_tag]: processing dock_state [dock_state].")

	switch(dock_state)
		if (STATE_DOCKING)	//waiting for our docking port to be ready for docking
			if(debOOg) testing("[id_tag]: entered STATE_DOCKING.")
			var/ready = ready_for_docking()
			if(debOOg) testing("[id_tag]: ready_for_docking() returned [ready].")
			if (ready)
				if(debOOg) testing( "[id_tag]: Ready for docking. Control mode is [control_mode]." )
				if (control_mode == MODE_CLIENT)
					if(debOOg)  testing( "[id_tag]: control mode is MODE_CLIENT. response_sent is [response_sent]." )
					if (!response_sent)
						testing( "[id_tag]: Client is sending confirmation to [tag_target]." )
					...
					if(debOOg) testing( "[id_tag]: Server is sending confirmation to [tag_target]" )
					...
					if (!override_enabled)
						if(debOOg) testing( "[id_tag]: Server finished docking" )
