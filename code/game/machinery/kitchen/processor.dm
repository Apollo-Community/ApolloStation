...
/obj/machinery/processor/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(src.processing)
		user << "<span class='alert'>The processor is in the process of processing.</span>"
		return 1
	if(src.contents.len > 0) //TODO: several items at once? several different items?
		user << "<span class='alert'>Something is already in the processing chamber.</span>"
		return 1
	var/what = O
	...
	if (!P)
		user << "<span class='alert'>That probably won't blend.</span>"
		return 1
...
/obj/machinery/processor/attack_hand(var/mob/user as mob)
	...
	if(src.contents.len == 0)
		user << "<span class='alert'>The processor is empty.</span>"
		return 1
